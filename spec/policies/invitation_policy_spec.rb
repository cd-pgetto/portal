...
RSpec.describe InvitationPolicy, type: :policy do
  ...
  let(:inviter) { create(:dr_sue, organization: organization, practices: [practice]) }
  let(:record) { build(:invitation, practice: practice, invited_by: inviter) }

  shared_examples "not permitted" do |user_description|
    context user_description do
  ...
  permissions :create?, :destroy? do
    ...
    context "as a practice member (not admin/owner)" do
      let(:user) { create(:another_user, organization: organization, practices: [practice]) }
      it { is_expected.not_to permit(user, record) }
    end
    ...
    context "as an admin of a different practice" do
      ...
      let(:user) { create(:another_user, organization: organization, practices: [other_practice]) }
