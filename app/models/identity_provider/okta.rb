...
#
class IdentityProvider::Okta < IdentityProvider::Dedicated
  ...
  def self.find_by_host(host)
    ...
    idp.is_a?(IdentityProvider::Okta) ? idp : nil
  end

  def self.setup(env)
    request = Rack::Request.new(env)
    identity_provider_id = env.fetch("omniauth.params", {}).merge(request.params)["identity_provider_id"]
    okta_idp = IdentityProvider::Okta.find_by(id: identity_provider_id) || find_by_host(request.host)
